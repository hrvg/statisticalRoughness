...
merge_Hurst_rasters <- function(Hurst_dir = file.path('F:/hguillon/research/exploitation/out/run127')){
	...
	L_id <- sapply(.lf, function(fn) unlist(strsplit(fn, "_"))[[4]] %>% 
		tools::file_path_sans_ext()) %>% as.numeric()
	tiled_file_df <- data.frame(path = lf, tile_id = tile_id, L_id = L_id) %>%
		dplyr::arrange(tile_id, L_id) %>%
		dplyr::group_by(L_id)
	for (l in unique(tiled_file_df$L_id)){
		raster_paths <- tiled_file_df %>% dplyr::filter(L_id == l) %>% dplyr::pull(path) %>% as.character()
		...
		projection(template) <- as.character(raster::crs(rasters[[1]]))
