...
get_zeta <- function(rstr, raster_resolution = 9.015, nbin = 20, .Hann = TRUE, .quantile_prob = c(0.9999), .prob = .999){
	...
	alpha_x <- get_all_alpha(hhcf_x, raster_resolution) %>% filter_alpha() %>% summarise_alpha()
	colnames(alpha_x) <- paste0(colnames(alpha_x), ".x")
	alpha_y <- get_all_alpha(hhcf_y, raster_resolution) %>% filter_alpha() %>% summarise_alpha()
	colnames(alpha_y) <- paste0(colnames(alpha_y), ".y")
	res <- dplyr::bind_cols(beta, alpha_x, alpha_y) %>% dplyr::mutate(
		zeta1 = get_zeta_(alpha_x$slope1_mode, alpha_y$slope1_mode),
		zeta2 = get_zeta_(alpha_x$slope2_mode, alpha_y$slope2_mode),
		theta = ang_fourier
		)
	return(res)
}