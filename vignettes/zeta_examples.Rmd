...
res_gabi <- get_zeta(gabilan_mesa) %>% dplyr::mutate_all(signifNA)
```
...
The figure below showcases the landscape in its original form (left) and smoothed with a Gaussian kernel of  `r res_gabi$rc %/% 10` standard deviations to match the value of $r_c$ (right).
...
res_yose <- get_zeta(yosemite) %>% dplyr::mutate_all(signifNA)
```
...
The figure below showcases the landscape in its original form (left) and smoothed with a Gaussian kernel of  `r res_yose$rc %/% 10` standard deviations to match the value of $r_c$ (right).
...
res_modo <- get_zeta(modoc) %>% dplyr::mutate_all(signifNA)
```
...
The figure below showcases the landscape in its original form (left) and smoothed with a Gaussian kernel of  `r res_modo$rc %/% 10` standard deviations to match the value of $r_c$ (right).
...
res_mrbl <- get_zeta(marble_canyon) %>% dplyr::mutate_all(signifNA)
```
...
The figure below showcases the landscape in its original form (left) and smoothed with a Gaussian kernel of  `r res_mrbl$rc %/% 10` standard deviations to match the value of $r_c$ (right).
...
```{r}
res <- rbind(res_gabi, res_yose, res_modo, res_mrbl) %>% 
  dplyr::select(-c("w", "xi", "alpha1", "alpha2"))
rownames(res) <- c("Gabilan Mesa", "Yosemite Valley", "Modoc Plateau", "Marble Canyon")
```
