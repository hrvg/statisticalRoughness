---
title: "Computing Fractal Dimension"
subtitle: "But at which Scales?"
author:
- name: Hervé Guillon
...
The original set of scales was defined from 5 seeds $s = \left\{ 2;3;5;7;9 \right\}$ (the primes between 2 and 10) and used as multiplier for the power of 2 from 0 to 13:
...
However, there are $5369$ numbers that satisfy the conditions I used to select $\left\{L_i\right\}_{i = 1}^{n_S}$ and $\left\{R_i\right\}_{i = 1}^{n_R}$ with $n_R = 5$ and 6 of them are smaller than the minimum surface size. This is important as it potentially leverages information at a finer spatial scale.
```{r warning = FALSE, message = FALSE, include = TRUE, echo = TRUE}
nR <- 5
...
So why use the 32 surface sizes defined in 2017? What is so peculiar about them?
...
Is this true for the new approach?

## Factor Distribution (Original)
...
## Factor Distribution (New)
The following plots identify the number $L$ and all its factors on the number line.
**The factors appears similarly well-behaved for the original and new approaches.**
```{r warning = FALSE, message = FALSE, include = TRUE, echo = TRUE}
numberline <- 1:Lmax
...
When indicated, the new approach is filtered so that $L$ are distributed logarithmically along the number line.
...
```
Of these surface sizes, two appeared in the original version: 192 and 1536.
**Quite drastically, using this new approach would divide by 5.3 the smaller scale at which fractal dimension is computed (120-m vs 640-m).**