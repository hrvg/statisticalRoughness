...
```{r}
.selected <- c("beta2", "alpha1.x", "alpha1.y", "alpha2.x", "alpha2.y", "zeta1", "zeta2", "w.x", "w.y", "inv.fc", "rc", "xi", "theta")
cparams <- data.frame(
  selected = .selected,
  n_sigma = rep(2, length(.selected)),
  lower = rep(TRUE, length(.selected)),
  upper = rep(TRUE, length(.selected)),
