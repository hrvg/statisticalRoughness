...
zeta_results <- read_zeta_raster(out_dir = results_directory, Lmax = 1E4, raster_resolution = 10, .len = 3)
zeta_results
```
...
```{r adjusting scales}
zeta_results$spatial_scales <- c(50, 60, 100) * 10
...
```{r make plots}
p_zeta <- make_all_plots(sliced_clamped_raster, zeta_results$spatial_scales, match("alpha1_mean.x", .selected), "alpha1_mean.x", begin = 0.1, end = 0.85, direction = 1, option = "inferno")
p_zeta
```
...
crossscale_correlations(
	...
	att_names = band_names,
	selected = .selected,
	spatial_scales = zeta_results$spatial_scales,
	corr_type = "spearman")
```