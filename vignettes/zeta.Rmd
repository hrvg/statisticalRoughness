...
```{r example_x_1}
get_alpha(hhcf_x$hhcf[ind_x, ], raster_resolution, do_plot = TRUE) %>% signif(2)
```

```{r example_y_1}
get_alpha(hhcf_y$hhcf[ind_y, ], raster_resolution, do_plot = TRUE) %>% signif(2)
```
...
A quick look at the distributions of the resulting roughness exponents values underline that in some cases $\alpha > 1$, and in rare occurences, $\alpha > 10$ or even $\alpha > 100$.
...
This large values happen more often for the second slopes and are filtered using the 99.9% quantile of the initial slope with `filter_alpha()`.
The resulting `data.frames` can easily be summarized with `summarise_alpha()`.
For example:

```{r summarize}
alpha_x %>% filter_alpha() %>% summarise_alpha()
alpha_y %>% filter_alpha() %>% summarise_alpha()
```

```{r summarize_hidden, echo = FALSE}
x_stats <- alpha_x %>% filter_alpha() %>% summarise_alpha()
y_stats <- alpha_y %>% filter_alpha() %>% summarise_alpha()
```
...
|:-----------|:-------------------------------------------------------------------------------:|:-------------------------------------------------------------------------------:|
| $\alpha_1$ | `r x_stats$slope1_mean %>% signif(2)` $\pm$ `r x_stats$slope1_sd %>% signif(2)` | `r y_stats$slope1_mean %>% signif(2)` $\pm$ `r y_stats$slope1_sd %>% signif(2)` |
| $\alpha_2$ | `r x_stats$slope2_mean %>% signif(2)` $\pm$ `r x_stats$slope2_sd %>% signif(2)` | `r y_stats$slope2_mean %>% signif(2)` $\pm$ `r y_stats$slope2_sd %>% signif(2)` |

These numbers mean that the topography is mainly correlated in a positive way below `r mean(c(x_stats$break1_mean, y_stats$break1_mean)) %>% signif(3)` meters and more correlated in the $x$-direction than in the $y$-direction as $\alpha_{1,x} > \alpha_{1,y} > 0.5$.
Above `r mean(c(x_stats$break1_mean, y_stats$break1_mean)) %>% signif(3)` meters, the topography is negatively correlated and more so in the y-direction than in the $x$-direction $\alpha_{2,y} < \alpha_{2,x} < 0.5$.
...
|           |                     anisotropy exponent, $\zeta$                      |
|:----------|:---------------------------------------------------------------------:|
| $\zeta_1$ | `r get_zeta_(x_stats$slope1_mean, y_stats$slope1_mean) %>% signif(3)` |
| $\zeta_2$ | `r get_zeta_(x_stats$slope2_mean, y_stats$slope2_mean) %>% signif(3)` |
