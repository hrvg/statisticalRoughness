...
gabilan_mesa <- raster(file.path(system.file("extdata/rasters/", package = "statisticalRoughness"), "yosemite.tif"))
raster_resolution <- 9.015
FT2D <- gabilan_mesa %>% detrend_dem() %>% raster::as.matrix() %>%
...
mean(hhcf_x$rms)
2 * mean(hhcf_x$rms)^2
mean(hhcf_x$autocorr_len)

mean(hhcf_y$rms)
2 * mean(hhcf_y$rms)^2
mean(hhcf_y$autocorr_len)
```
...
|           |                                          anisotropy exponent, $\zeta$                                          |
|:----------|:--------------------------------------------------------------------------------------------------------------:|
| $\zeta_1$ | `r get_zeta_(x_stats$alpha1_mean, y_stats$alpha1_mean, x_stats$alpha1_IQR, y_stats$alpha1_IQR) %>% signifNA()` |
| $\zeta_2$ | `r get_zeta_(x_stats$alpha2_mean, y_stats$alpha2_mean, x_stats$alpha2_IQR, y_stats$alpha2_IQR) %>% signifNA()` |

```{r}
get_zeta(gabilan_mesa, full = TRUE) %>% dplyr::mutate_all(signifNA)

```


```{r}
get_zeta(gabilan_mesa) %>% dplyr::mutate_all(signifNA)
```