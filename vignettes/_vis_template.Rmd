...
```{r reduce noise, eval = REDUCE_NOISE}
sliced_clamped_raster <- reduce_spatial_noise(sliced_clamped_raster)
```

```{r find_par_perp, echo = FALSE, eval = PERP_PAR}
...
if ("theta_perp" %in% .selected) sliced_clamped_raster  <- sliced_clamped_raster  %>% make_angular(match("theta_perp", .selected))
