...
if (region == "california"){
  spatial_scales <- get_all_R_L(1e4, 5, len = 32)$allL[c(6, 10, 16)]
} else {
  ...
  spatial_scales <- spatial_scales[spatial_scales > LIMIT_SCALE & spatial_scales < 100] 
}

zeta_results$spatial_scales <- spatial_scales * 10
...
# if ("theta.x" %in% .selected) sliced_clamped_raster  <- sliced_clamped_raster  %>% make_angular(match("theta.x", .selected), mode = "sin2")
...
```{r}
ind <- four_values_check(sliced_clamped_raster, .selected)
...
is_band1_gtlt_band2 <- function(band1, band2, rasters, .selected, spatial_scales, option = "gt"){
...
res <- foreach(var = vars, opt = opts) %do% {
	is_band1_gtlt_band2(var, gsub(".x", ".y", var), sliced_clamped_raster, .selected, spatial_scales, option = opt)
}
cat(unlist(res), sep = '\n')
```

## Comparing $\perp-$ and $\parallel-$ variables

```{r, echo = FALSE, results = "asis", eval = PERP_PAR}
vars <- .selected[grepl("_perp", .selected)]
vars <- vars[!grepl("zeta", vars)]
opts <- sapply(grepl("theta", vars), function(x) ifelse(x, "lt", "gt"))
res <- foreach(var = vars, opt = opts) %do% {
  is_band1_gtlt_band2(var, gsub("_perp", "_par", var), sliced_clamped_raster, .selected, spatial_scales, option = opt)
}
cat(unlist(res), sep = '\n')
```
